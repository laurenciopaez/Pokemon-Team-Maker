...
class RegisterForm extends React.Component {
    ...
    if(e.target.name === "password1" || e.target.name === "password2"){   //guardo las contrase;as en el control state
        await this.setState({
            control: {
                ...this.state.control,
        ...
        if(this.state.control.password1 === this.state.control.password2) {   //comparo las contrase;as y si son iguales las guardo en el state form
            await this.setState({
                form: {
                    ...this.state.form,
        ...
        if (e.target.name === 'username'){   //si la variable es username, lo guardo en control
            await this.setState({
                control: {
                    ...this.state.control,
                    [e.target.name]: e.target.value
            ...
            await this.setState({    //sino va a ser el mail
                form: {
                    ...this.state.form,
                    [e.target.name]: e.target.value
                }
    ...
    registrarse = async() => {
        ...
        await axios.post(baseUrl, {username: username, email: email, password: password} )
    ...
    validacionUsername = () => {    //validacion del username
