...
class Main extends React.Component {
  ...
  mostrarBaseDeDatos = async () => {
    ...
    try {
      ...
      if (
        response.data.pokemones_fav &&
        Array.isArray(response.data.pokemones_fav)
      ) {
        for (let i = 0; i < response.data.pokemones_fav.length; i++) {
          const { id, name, image } = response.data.pokemones_fav[i];

          const existingPokemon = this.state.pokemones_favE.find(
    ...
    return (
      <>
        <div className={background.pageWrapper}>
          <div
            className={background.backgroundImage_main}
            style={{ height: "100vh" }}
          >
            <NavegationBar />
