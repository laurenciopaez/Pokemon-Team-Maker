...
//Pido a la api los pokemones y los guardo en un estado local
    cargarOpciones = async () => {
        ...
        if(this.state.pokemones.length === cantidadPokemones){    //acelerar el proceso de carga de datos
        } else {
          try {
      ...
      actualizarDb = async () => {
        const { id: userId, username: userName, password: userPassword, email: userEmail } = cookies.getAll();
        
        const updatedBDpokemons = []
       ; //array vacio que guarda la coleccion de pokemons
    ...
    render(){
        return(
            ...
            <div className={background.backgroundImage_allopts}  style={{height: '100vh'}}>
            <NavegationBar/>
            <div className='mx-auto row' style={{width: '80vw',height: '80vh', overflow:'auto',flexbox:'box'}}>
